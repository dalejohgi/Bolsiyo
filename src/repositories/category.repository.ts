...
> {
  ...
  async createNewCategory(
    ...
    const {code, name, description, isActive} = categoryToCreate;
  ...
  async activateCategory(id: number): Promise<any> {
    return this.execute('UPDATE categories SET isActive = true WHERE id = ?', [
      id,
    ]);
  }

  async inactivateCategory(id: number): Promise<any> {
    return this.execute('UPDATE categories SET isActive = false WHERE id = ?', [
      id,
    ]);
  }
}
